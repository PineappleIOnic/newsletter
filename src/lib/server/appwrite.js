 ...
 * @returns
 */
export function getSessionClient(sessionId) {
	let client = new Client();
...
export async function createVerificationCode(userId) {
	...
	try {
		let existingCodes = await database.listDocuments('newsletter', 'verification', [
			Query.equal('userId', userId)
		]);

		if (existingCodes.documents.length > 0) {
			await database.deleteDocument('newsletter', 'verification', existingCodes.documents[0].$id);
		}
	} catch (err) {
		if (err instanceof AppwriteException) {
	...
	try {
		let document = await database.createDocument('newsletter', 'verification', ID.unique(), {
			userId
		});
...
export async function verifyCode(code) {
	...
	try {
		let verifyDoc = await database.getDocument('newsletter', 'verification', code);

		await users.updateEmailVerification(verifyDoc.userId, true);

		await database.deleteDocument('newsletter', 'verification', verifyDoc.$id);
...
async function createVerificationCollection(databaseId) {
	...
	await database.createStringAttribute(databaseId, collection.$id, 'userId', 256, true);

	// Await for the attributes to be created
	await new Promise((resolve) => setTimeout(resolve, 2000));

	await database.createIndex(databaseId, collection.$id, 'userId', IndexType.Unique, ['userId']);
